...
export async function fetchOrdersForSales() {
  try {
    ...
    if (!response.ok) throw new Error('Error fetching orders for sales analysis');
...
function calculateMonthlySales(orders) {
  ...
  orders.forEach(order => {
    ...
    monthlySales[monthIndex] += (order.TotalPriceSale > 0 ? order.TotalPriceSale : order.TotalPrice);
  });
...
function calculateWeeklySales(orders) {
  ...
  recentOrders.forEach(order => {
    ...
    weeklySales[dayIndex] += (order.TotalPriceSale > 0 ? order.TotalPriceSale : order.TotalPrice);
  });
...
function calculateQuarterlySales(orders) {
  ...
  orders.forEach(order => {
    ...
    quarterlySales[quarterIndex] += (order.TotalPriceSale > 0 ? order.TotalPriceSale : order.TotalPrice);
  });
...
function calculateYearlySales(orders) {
  ...
  orders.forEach(order => {
    ...
    if (yearIndex !== -1) {
      yearlySales[yearIndex] += (order.TotalPriceSale > 0 ? order.TotalPriceSale : order.TotalPrice);
    }
  });
...
function calculateDailySales(orders) {
  ...
  recentOrders.forEach(order => {
    const orderDate = new Date(order.OrderDate);
    const dayIndex = orderDate.getDay(); // 0 for Sunday, 1 for Monday, etc.
    dailySales[dayIndex] += (order.TotalPriceSale > 0 ? order.TotalPriceSale : order.TotalPrice);
  });
...
async function calculateCategorySales(orders) {
  try {
    ...
    console.log("Product categories map created:", Object.keys(productCategories).length);
    ...
    orders.forEach(order => {
      ...
      if (!hasProcessedProducts) {
        const saleAmount = order.TotalPriceSale > 0 ? order.TotalPriceSale : order.TotalPrice;
...
export async function calculateSalesStats() {
  try {
    ...
    const totalRevenue = orders.reduce(
      (acc, order) => acc + (order.TotalPriceSale > 0 ? order.TotalPriceSale : order.TotalPrice),
      0
    );
    ...
    const completedOrders = orders.filter(order => order.OrderStatusId === 3 || order.OrderStatusId === 4).length;

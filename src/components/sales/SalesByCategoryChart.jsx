...
const COLORS = ["#4F46E5", "#10B981", "#F59E0B", "#EF4444", "#6366F1", "#14B8A6", "#F97316", "#8B5CF6"];
		...
		const fetchCategorySalesData = async () => {
			try {
				...
				if (stats.categorySalesData && stats.categorySalesData.length > 0) {
					...
					const filteredData = stats.categorySalesData.filter(item => item.value > 0);
					console.log("Filtered category data:", filteredData);
					
					// Verify total matches
					const categoryTotal = filteredData.reduce((sum, item) => sum + item.value, 0);
					console.log("Category total:", categoryTotal, "Total revenue:", stats.totalRevenue);

...
const UsersTable = () => {
	...
	const handleSearch = (e) => {
		...
		const filtered = customers.filter(
			(customer) => customer.fullname.toLowerCase().includes(term) || customer.email.toLowerCase().includes(term)
		);
	...
	return (
		...
		>
			...
			<div className='flex justify-center mt-4'>
				<button onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))} className='px-4 py-2 mx-2 bg-gray-700 text-white rounded-lg'>Previous</button>
				<span className='text-white px-4 py-2'>{currentPage} / {totalPages}</span>
				<button onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))} className='px-4 py-2 mx-2 bg-gray-700 text-white rounded-lg'>Next</button>
			</div>
		</motion.div>
	);
};
